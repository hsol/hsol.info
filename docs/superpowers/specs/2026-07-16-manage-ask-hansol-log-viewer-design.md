...
데스크톱 3단. **LNB 가 2단으로 쪼개진다** — 앞으로 다른 관리 기능이 들어올 자리를 위해서다.

```text
┌──────────┬────────────────────────┬────────────────────────┐
│ 전체 LNB │ 세션 목록 (2단 LNB)     │ 대화 패널 (chat UI)     │
│ (기능)   │                        │                        │
│          │ 7/15 14:38             │  ┌──────────────────┐  │
│ ▸ Ask 로그│ 문답 1회                │  │ 포트폴리오에서 … │  │ ← 질문
│          │ 마지막 답변 앞부분…      │  └──────────────────┘  │
│ (향후    │ ──────────────────     │                        │
│  기능이  │ 7/14 17:37          ★  │  ┌──────────────────┐  │
│  여기    │ 문답 12회 ⚠ 답변 1 누락  │  │ 몇 가지를 꼽자면…│  │ ← 답변
│  쌓인다) │ 마지막 답변 앞부분…      │  │ ★★★★☆ "정확했어요"│ │
│          │ ──────────────────     │  └──────────────────┘  │
│ ─────    │                        │                        │
│ 임한솔   │   ‹ 이전   2/3   다음 › │                        │
│ 로그아웃 │                        │                        │
└──────────┴────────────────────────┴────────────────────────┘
   layout.tsx           page.tsx (Ask 로그 기능)
```

### 1단 — 전체 LNB 는 `manage/layout.tsx` 가 맡는다

**프레임워크가 이미 이 모양을 위해 가진 것을 쓴다.** App Router 의 `src/app/manage/layout.tsx` 에
전체 LNB 를 두면 앞으로 `/manage/<기능>` 라우트를 추가할 때마다 **자동으로 같은 셸을 입는다.**
1단을 page 안에 그리면 기능이 늘 때마다 복붙하거나 나중에 뜯어고쳐야 한다.

- 담기는 것: 기능 목록(지금은 `Ask 로그` 1개) + 하단에 로그인 사용자·로그아웃.
- **로그인 사용자 표시·로그아웃 폼을 `page.tsx` 에서 layout 으로 옮긴다.** 그건 특정 기능이 아니라
  관리 콘솔 공통 셸이다. 그 결과 `page.tsx` 는 순수하게 뷰어만 남는다.
- 현재 활성 기능은 `usePathname` 대신 layout 이 서버 컴포넌트임을 유지하기 위해
  각 항목을 `<Link>` 로 두고 활성 표시는 해당 page 가 아니라 **경로 기반**으로 판단한다.
  지금은 항목이 1개라 활성 표시가 무의미하므로 **기능이 2개 이상 될 때 붙인다**(YAGNI).

지금 항목이 하나뿐이라 1단이 허전하지만, 사용자가 확장을 명시적으로 요구했고
layout 분리 비용이 파일 하나(~30줄)라 지금 자리를 잡아둔다.
...
3단 레이아웃을 모바일에 접어 넣지 않는다 — 관리 콘솔이고, 사용자가 데스크톱 전용을 명시했다.
CSS 미디어쿼리만 사용, JS 없음.
...
| 파일 | 역할 | 종류 |
| --- | --- | --- |
| `src/app/manage/layout.tsx` | **1단 전체 LNB** + 로그인 사용자·로그아웃 (모든 관리 기능 공통 셸) | 서버 |
| `src/app/manage/page.tsx` | searchParams → 두 쿼리 → 2단 세션 목록 + 3단 대화 렌더, `chatdock.css` import | 서버 |
| `src/app/manage/copy-permalink.tsx` | 클릭 시 클립보드 복사 + "복사됨" | **클라이언트** |
| `src/app/manage/scroll-to-bottom.tsx` | hash 없을 때만 맨 아래로 | **클라이언트** |
| `src/middleware.ts` (수정) | `/manage` 응답에 `X-Robots-Tag` 헤더 추가 | 엣지 |

클라이언트 컴포넌트는 저 둘뿐이다. 나머지는 전부 서버 렌더 + 브라우저 기본 동작 + CSS.

**기존 `page.tsx` 에서 옮겨지는 것:** 로그인 사용자 표시·로그아웃 폼 → `layout.tsx`
(기능이 아니라 공통 셸이므로). `dynamic = "force-dynamic"` 은 유지하고, Tailwind 중립 색은
블루프린트 변수로 교체한다(위 UI 절). 파일이 커지면 세션 목록·대화 패널을 같은 폴더 안
서버 컴포넌트로 분리한다.
...
## 봇 차단 — `/manage` 이하는 봇이 존재도 모르고 접근도 못한다

요구를 **접근(access)** 과 **발견(discovery)** 두 축으로 나눠 현황을 실측했다.

### 접근 — 이미 완료 (추가 작업 없음)

미들웨어가 `/manage` 와 모든 하위 경로를 유효 세션 쿠키 없이는 통과시키지 않고
`/api/auth/authorize` 로 리다이렉트한다. **봇은 페이지 내용을 볼 수 없다.** 크롤러가 URL 을
알아내 요청해도 얻는 것은 Vercel 로그인으로 가는 리다이렉트뿐이다.

### 발견 — 대부분 이미 막혀 있음 (실측 확인)

| 노출 경로 | 현황 |
| --- | --- |
| sitemap | `SITE_STRUCTURE.inSitemap` 이 SSOT — `/manage` 없음 ✅ |
| 사이트 내부 링크 | `/manage` 로 가는 링크 **0개** ✅ (grep 결과 `googletagmanager` 오탐 2건뿐) |
| robots.txt | `/manage` 언급 없음 ✅ — **이게 맞다. 아래 참조** |

### robots.txt 에 `Disallow: /manage` 를 쓰지 않는다 (중요)

**robots.txt 는 누구나 읽는 공개 파일이다.** 거기에 `Disallow: /manage` 를 적는 순간
"이 사이트에 관리자 페이지가 있다"고 **전 세계에 광고**하는 셈이다. 요구는 "존재도 모르게" 이므로
robots.txt 로 막는 것은 요구에 정면으로 어긋난다. 지금처럼 **언급하지 않는 상태를 유지**한다.

(robots.txt 는 애초에 크롤링 방지 수단이지 접근 통제 수단이 아니다. 접근 통제는 위의 미들웨어
게이트가 이미 하고 있다.)

### 추가할 것 — `X-Robots-Tag` 응답 헤더 (유일한 신규 작업)

미들웨어의 `/manage` 분기에서 응답에 헤더를 붙인다:

```ts
// 게이트 통과 응답·리다이렉트 응답 양쪽에 붙인다
res.headers.set("X-Robots-Tag", "noindex, nofollow, noarchive");
```

- robots.txt 와 달리 **공개적으로 존재를 드러내지 않으면서** 색인을 금지한다.
- 미들웨어가 경로 접두(`/manage`)로 판정하므로 **앞으로 추가될 `/manage/<기능>` 라우트까지
  자동으로 덮인다.** 각 페이지에 metadata 를 일일이 다는 방식은 새 라우트마다 빠뜨릴 수 있다.
- 보강으로 `manage/layout.tsx` 에 Next.js 네이티브 metadata 를 함께 선언한다
  (`robots: { index: false, follow: false }`). 봇은 게이트 때문에 이 HTML 을 볼 일이 없어
  실질 효과는 헤더가 내지만, 의도를 코드에 명시해 두는 값이 있다.

### 한계 (알고 넘어간다)

리다이렉트 응답 자체는 "그 경로에 뭔가 있다"는 사실을 숨기지 못한다. 완전히 숨기려면
비인증 요청에 404 를 반환해야 하는데, 그러면 **정상 관리자도 로그인 화면을 못 만난다.**
접근이 이미 차단돼 있고 내용이 새지 않으므로 여기서 멈춘다.
