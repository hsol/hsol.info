...
- GitHub Actions 워크플로 `build-with-vault-refresh.yml`은 체크아웃된 서브모듈 vault를 그대로 쓴다. `content:refresh:claude`로 그 안의 사이트 데이터 JSON을 갱신한 뒤, CI는 서브모듈 커밋을 만들어 `hsol-info-blob` 원격 `main` 등(워크플로 `SUBMODULE_BRANCH`)으로만 푸시한다. **vault ↔ Vercel Blob 중 업로드는 오직** `hsol-info-blob` 저장소의 `Sync vault to Vercel Blob` 워크플로만 한다(저장소 시크릿 `BLOB_READ_WRITE_TOKEN` 필요). **Blob → 로컬 vault 내려받기**도 같은 저장소에서 `npm run sync:blob:pull`로만 한다. `GIT_DIFF_BASE_SHA`/`GIT_DIFF_HEAD_SHA`는 refresh 스킵 판단 등에 쓴다. `content:refresh:claude` 실패 시 `generated/content-refresh-failures/`를 아티팩트로 올린다.
