...
- GitHub Actions 워크플로 `build-with-vault-refresh.yml`은 `hsol-info-blob/vault`와 `hsol-info-blob/vault/.blob-sync-state.json`을 `actions/cache`로 복원해 Blob 풀싱크를 줄이고, `content:refresh:claude` 실패 시 `generated/content-refresh-failures/`를 아티팩트로 올린다.
