/* hsol.info v2 — atoms (architectural) */
const { useState, useEffect, useRef, useCallback } = React;

// ---------- Title plate (drafting cartouche) ----------
function Plate() {
  const d = window.HSOL_DATA.identity;
  return (
    <div className="plate">
      <div className="plate-cell plate-id">
        <img src="assets/signature.svg" alt="" className="plate-sig" />
        <div>
      ...
      <a href={d.calendly} target="_blank" rel="noopener" className="plate-cell plate-cell-link">
        <div className="plate-key">Status</div>
        <div className="plate-status"><span className="dot"></span>Open for coffee</div>
      </a>
    </div>
  );
}
...
function PlanDiagram({ onPick, hovered, onHover }) {
  const rooms = [
    { key: "hire",    x: 20,  y: 20,  w: 180, h: 160, lx: 50,  ly: 55,  ax: 50,  ay: 68,  label: "01 / HIRE",    area: "— 26.4 m²",
      // furniture: a desk + chair (interview room)
      furniture: <g>
    ...
    { key: "collab",  x: 200, y: 20,  w: 140, h: 260, lx: 225, ly: 55,  ax: 225, ay: 68,  label: "02 / COLLAB",  area: "— 38.0 m²",
      // furniture: long table for collab room
      furniture: <g>
    ...
    { key: "builder", x: 200, y: 280, w: 140, h: 160, lx: 225, ly: 305, ax: 225, ay: 318, label: "03 / BUILDER", area: "— 22.4 m²",
    ...
    { key: "curious", x: 120, y: 280, w: 80,  h: 160, lx: 130, ly: 305, ax: 130, ay: 318, label: "04 / CURIOUS", area: "— 14.4 m²",
      // furniture: lounge chairs (curiosity room)
      furniture: <g>
  ...
  return (
    <div className="plan">
      ...
      <svg className="plan-svg" viewBox="0 0 360 460" preserveAspectRatio="xMidYMid meet" xmlns="http://www.w3.org/2000/svg">
        {/* outer wall */}
        ...
        {/* INTERACTIVE room hit areas + fills */}
        {rooms.map(r => {
          const active = hovered === r.key;
          return (
            <g key={r.key}
               onClick={() => onPick && onPick(r.key)}
              ...
              {/* furniture / section detail — fades in on hover */}
              <g style={{
                opacity: active ? 1 : 0,
        ...
        {/* door openings (gaps in walls) — drawn after rooms so they sit on top */}
        ...
        {/* center waypoint — entrance */}
        <g pointerEvents="none">
          <circle cx="155" cy="225" r="3" fill="#f4c977" />
          <circle cx="155" cy="225" r="9" fill="none" stroke="#f4c977" strokeWidth="0.6" opacity="0.7" />
          <text x="170" y="228" fill="#f4c977" fontFamily="JetBrains Mono, monospace" fontSize="8" letterSpacing="1">ENTRY</text>
          {/* dimension line */}
          ...
          <text x="180" y="448" fill="#5b819a" fontFamily="JetBrains Mono, monospace" fontSize="7" textAnchor="middle">10 yr · since 2014</text>
        </g>
      </svg>
    </div>
  );
...
function Foot() {
  ...
  return (
    <footer className="foot">
      <div>© {new Date().getFullYear()} · {d.location}</div>
      ...
      <div className="foot-links">
        ...
        <a href={d.calendly} target="_blank" rel="noopener">Calendly</a>
...
// ---------- Section heading ----------
function SecHead({ title, num, meta }) {
  return (
...
// ---------- CTA — Calendly inline widget ----------
function CoffeeCTA({ title, sub }) {
  const d = window.HSOL_DATA.identity;
  const ref = React.useRef(null);

  React.useEffect(() => {
    // Load Calendly assets once globally
    ...
    const ensureWidget = () => {
      ...
      window.Calendly.initInlineWidget({
        url: d.calendly + '?hide_gdpr_banner=1&background_color=0e2a3d&text_color=f2f7fa&primary_color=7fb4d0',
  ...
  return (
    <div className="cta cta-calendly">
      <div className="cta-head">
        ...
        <p className="cta-sub">{sub || "채용·창업·협업·그냥 궁금함 — 어떤 주제든 환영합니다. 아래 캘린더에서 시간을 잡아주세요."}</p>
      </div>
      <div className="cta-cal" ref={ref} aria-label="Calendly scheduling widget" />
      <a href={d.calendly} target="_blank" rel="noopener" className="cta-fallback">
        새 창으로 calendly 열기 →
      </a>
    </div>
  );
...
function Pillars() {
  return (
    <div className="pillars">
      {window.HSOL_DATA.pillars.map((p, i) => (
...
Object.assign(window, { Plate, PlanDiagram, Foot, SecHead, CoffeeCTA, Back, CareerList, Pillars });
