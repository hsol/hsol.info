...
[여백·리듬 — 중요(빽빽하지 않게)]
- 섹션 사이에 넉넉한 세로 여백을 둔다(섹션 블록 간 약 18~26px). 섹션 안 항목 사이에도 호흡을 준다(약 10~14px).
- 본문 line-height 1.5 이상. 헤딩 위쪽 여백을 충분히. "촘촘한 워드 문서"가 아니라 "여백이 살아있는 편집물"로.
...
- 절제된 컬러 시스템: 잉크(거의 검정 #1b1b1b), 보조 회색(#5a5a5a), 악센트 1색(딥 블루 #1f5e80 계열)은 1~2곳에만. 과한 색·과한 굵기 금지.
- 표/박스 테두리로 칸을 나누는 "양식지" 느낌을 피한다. 구분은 여백과 얇은 헤어라인(1px 연한 회색/악센트)으로.
- 스킬은 칩(둥근 모서리, 연한 배경) 또는 깔끔한 인라인 나열. 헤더는 좌(이름·포지셔닝)/우(연락처)로 균형 있게.
...
6) 대표 프로젝트 — **회사/사업 중심**으로 서술한다(예: PPB·Proofer·Toss·RIDI 등에서 맡은 제품·플랫폼). 개인 사이드 프로젝트(hsol.info 포트폴리오, 책, 개인 뉴스레터 등)는 **최대한 제외**한다. 각 프로젝트는 소속 회사·역할·**사업 임팩트(돈/가치)**를 드러낸다. 기준 통과 항목이 적으면 2~3개라도 무방.
7) 학력·자격·언어.
...
- **역할 정확히**: 피피비스튜디오스에서 임한솔의 역할은 **플랫폼팀 팀장**이다. 회사명 옆에 "(인수창업/서치펀드)" 같은 라벨을 달아 **임한솔이 개인적으로 인수한 것처럼 보이게 하지 않는다.** 서치펀드/인수 맥락을 언급할 경우 반드시 "코파운더와 함께" 같이 공동임을 분명히 하고, 본인 직함은 팀장으로 정확히 쓴다. 다른 조직도 vault에 적힌 실제 직함·관계만 쓴다(과대 표기 금지).
