...
function useTweaks<T extends Record<string, unknown>>(defaults: T) {
    ...
    (keyOrEdits: keyof T | Partial<T>, val?: T[keyof T]) => {
      const edits =
        typeof keyOrEdits === "object" && keyOrEdits !== null
          ...
          : ({ [String(keyOrEdits)]: val } as Partial<T>);
...
function TweaksPanel({
  ...
  const [railEnabled, setRailEnabled] = useState(() => {
    ...
    const deck = document.querySelector("deck-stage") as unknown as {
      _railEnabled?: boolean;
    } | null;
    return !!deck?._railEnabled;
  });
  useEffect(() => {
