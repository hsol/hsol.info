...
- **저라면 이렇게 접근해요**: 이 상황을 어떻게 바라보고 무엇부터 손대는지 2~4개 불릿. 각 항목은 제 사고방식에서 자연스럽게 흘러나온 판단처럼, 실제로 말하듯 다양한 문장으로 풀어 쓰세요(추상적 원칙 이름을 앞세워 근거를 대는 식이 아니라, 이 상황에서 제가 실제로 하는 생각과 행동으로).
...
export async function POST(req: Request) {
  try {
    ...
    if (sessionId) {
      messageId = await persistAdviceExchange(sessionId, issue, answer);
